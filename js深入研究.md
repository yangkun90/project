...
fn(5)(6);//12
fn(7)(8);//16
f(4);//这里i是上级作用域4+4=8 f(3)执行过一次，这个时候私有i已经变成3了,又增加1所以变成4了
...
```javascript
函数执行成一个私有的作用域，保护里面的私有变量不受外界的干扰，这种保护机制称之为闭包。
常用说法：形成一个不销毁的私有作用域才是闭包。
闭包：柯里化函数
function fn(){
    return function(){
...
//真实的项目中为了保证js的性能应该减少闭包的使用
//不销毁的栈内存会增加内存的消耗
//1。闭包具有保护作用：保护私有变量不受外界影响
//在真实项目中 ，尤其是团队协作开发的时候，应该尽可能的减少全局变量的使用，防止冲突（全局变量污染),那么我们可以放到闭包中，让全局变量转换为私有变量
(function(){
    var n =12;
    function fn(){
...
//不仅如此，我们封装插件的时候，也会把自己的程序都存放到闭包中保护起来，防止和用户的程序冲突，但是我们又需要暴露一些方法给客户使用。这样如何处理？
//1. jquery 方式：把需要的方法抛到全局
(function(){
    function jQuery(){
        
    }
    window.jQuery=window.$=jQuery;
    //把需要提供外面使用的方法，通过给window设置属性的方式暴露出去
})();
jQuery();
$();
//2.zepto 方式，基于RETURN 把需要共外面使用的方法暴露出去
var Zepto=(function(){
    return {
        xxx:function(){
...
//2.闭包具有保存作用：形成不销毁的栈内存，把一些值保存下来，方便后面调取使用
