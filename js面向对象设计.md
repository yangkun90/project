...
> 使用碎片化文档处理
>
> `createDocumentFrgment`  可以先把节点存放到这个对象里，然后一次性添加到需要添加的对象里，这样减少了DOM回流次数，是一种优化方式
>
> 读写分离就是将读和写分开：
>
> 比如获取一个节点的样式，我们可以把所有的获取语句在一起完成，完成后再进行写入、修改操作。因为执行同一种操作的时候有缓存的效果，这样可以减少DOM重绘事件，如果读写交叉，会造成缓存刷新，每次都是重新生成缓存。
